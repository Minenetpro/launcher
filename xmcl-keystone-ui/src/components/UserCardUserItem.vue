<template>
  <v-list-item v-if="user" :link="link">
    <v-list-item-avatar>
    ...
    <v-list-item-content>
      ...
      <v-list-item-subtitle>
        <v-icon small color="blue"> verified </v-icon>
        <span>
          {{ authority }}
          ({{ t("user.authService") }})
        </span>
      ...
      <v-list-item-subtitle>
        <v-icon small :color="expired ? 'red' : 'primary'">
        ...
        <template v-else>
          <span :style="{ color: getColorCode(expired ? 'red' : 'primary') }">
            {{ expired ? t("user.tokenExpired") : t("user.tokenValidUntil") }}
          </span>
          <v-icon small :color="expired ? 'red' : 'primary'">
            all_inclusive
    ...
    <v-list-item-action v-if="controls">
      <v-btn text color="red" @click="emit('remove')">
...
const userNameText = computed(() =>
  props.hideUserName ? maskTheEmail(props.user.username) : props.user.username
);
const { t } = useI18n();
