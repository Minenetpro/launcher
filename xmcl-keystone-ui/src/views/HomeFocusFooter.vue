<template>
  <div class="w-full">
    <div class="relative mx-6 flex-grow flex gap-6 items-end pb-1">
      <!-- <v-card
        flat
      ...
      >
        ...
        >
          ...
          <v-btn icon small @click="onViewDashboard">
            <v-icon class="rotate-[45deg]"> unfold_more </v-icon>
          </v-btn>
        </div>
        <v-tabs-items
          ref="tabItems"
        ...
        >
          ...
          <v-tab-item v-if="headerData">
            <HomeUpstreamHeader :value="headerData" dense />
          </v-tab-item>
        </v-tabs-items>
        <v-tabs v-model="tab" :show-arrows="true" class="tabs">
          ...
          <v-tab v-if="headerData">
            {{
              instance.upstream &&
              instance.upstream.type === "curseforge-modpack"
                ? "Curseforge"
                : "Modrinth"
            }}
          </v-tab>
      ...
      >
        ...
        <v-btn
          ...
          class="rounded-full expand-button ml-4"
          icon
          x-large
        >
          <v-icon class="rotate-[45deg]"> unfold_more </v-icon>
...
const { data: project } = useSWRVModel(
  getCurseforgeProjectModel(
    computed(() =>
      instance.value.upstream?.type === "curseforge-modpack"
        ? Number(instance.value.upstream.modId)
        : undefined
    )
  )
);
const curseforgeHeaderData = useCurseforgeUpstreamHeader(project);
const { data: modrinthProject } = useSWRVModel(
  getModrinthProjectModel(
    computed(() =>
      instance.value.upstream?.type === "modrinth-modpack"
        ? instance.value.upstream.projectId
        : undefined
    )
  )
);
