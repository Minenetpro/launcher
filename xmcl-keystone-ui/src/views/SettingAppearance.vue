  ...
  <div>
    ...
    <v-list-item class="items-center justify-center">
      <v-list-item-action class="self-center">
        <v-tooltip color="warning" outlined right>
          <template #activator="{ on }">
            <v-btn text icon v-on="on" @click="resetToDefault">
      ...
      <v-list-item-action app-bar class="ml-[16px]">
      ...
      <v-list-item-action sidebar class="ml-[16px]">
        <SettingAppearanceColor
      ...
      <v-list-item-action primary class="ml-[16px]">
        <SettingAppearanceColor
      ...
      <v-list-item-action secondary class="ml-[16px]">
        <SettingAppearanceColor
      ...
      <v-list-item-action warning class="ml-[16px]">
        <SettingAppearanceColor
          v-model="warningColor"
          :text="t('setting.colorTheme.warningColor')"
        />
      </v-list-item-action>
      <v-list-item-action error class="ml-[16px]">
        <SettingAppearanceColor
          v-model="errorColor"
    ...
    <!-- Background options removed: background is now fixed to an HLS stream -->
    <!-- <v-list-item>
      <v-list-item-content>
        <v-list-item-title>
          {{
    ...
    <v-list-item>
      ...
      <div class="flex flex-grow-0 gap-1 mr-2">
        <v-btn-toggle v-model="fontDelta" mandatory solo dense>
          <v-btn solo class="h-unset!"> 1px </v-btn>
          <v-btn solo class="h-unset!"> 0.1px </v-btn>
        </v-btn-toggle>
        <v-btn icon @click="onFontSizeDecrease">
        ...
        <v-text-field
          ...
          hide-details
        />
        <v-btn icon @click="onFontSizeIncrease">
          <v-icon> text_increase </v-icon>
        </v-btn>
      </div>

      <v-btn outlined text style="margin-right: 10px" @click="onSelectFont">
        {{ t("setting.themeSelectFont") }}
      </v-btn>
      <v-btn outlined text style="margin-right: 10px" @click="onRevertFont">
    ...
    <v-list-item>
      ...
      <v-btn outlined text style="margin-right: 10px" @click="onExportTheme">
        {{ t("setting.themeExport") }}
      </v-btn>
      <v-btn outlined text style="margin-right: 10px" @click="onImportTheme">
...
const backgroundTypes = computed(() => [
  ...
  {
    value: BackgroundType.PARTICLE,
    text: t("setting.backgroundTypes.particle"),
  },
