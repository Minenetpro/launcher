<template>
  ...
  >
    <span v-if="back" class="flex shrink grow-0 p-0">
      <div v-if="shouldShiftBackControl" style="width: 80px" />
      <v-icon
        v-ripple
        small
    ...
    <AppAudioPlayer v-if="!noDebug" class="ml-22" />
    <div class="grow" />
    ...
    />
    <AppSystemBarAvatar v-if="!noUser" />
    <AppSystemBarBadge
      id="settings-button"
      icon="settings"
      :text="t('setting.name', 2)"
      can-hide-text
      @click="gotoSettings"
    />
    <!-- <AppSystemBarBadge
