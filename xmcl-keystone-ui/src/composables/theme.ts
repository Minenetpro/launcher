...
export const kTheme: InjectionKey<ReturnType<typeof useTheme>> =
  Symbol("theme");
...
export function useTheme(
  ...
  const selectedThemeName = useLocalStorageCacheStringValue(
    "selectedThemeName",
    "default" as string
  );
  ...
  const backgroundMusic = computed(
    () => currentTheme.value.backgroundMusic ?? []
  );
  ...
  async function readTheme(name: string) {
    let theme = await getTheme(name).then((v) =>
      v ? deserializeV0(v) : undefined
    );
    if (!theme) {
      theme = loadV1Theme();
    }
    ...
    theme.colors.darkSideBarColor = ensureRGBAHex(
      theme.colors.darkSideBarColor
    );
    ...
    theme.colors.lightAppBarColor = ensureRGBAHex(
      theme.colors.lightAppBarColor
    );
    theme.colors.lightSideBarColor = ensureRGBAHex(
      theme.colors.lightSideBarColor
    );
  ...
  const flushWrite = debounce((name: string, theme: UIThemeDataV1) => {
    setTheme(name, serialize(theme)).then(
      () => update,
      () => {}
    );
  }, 800);
  ...
  :root {
    ...
    --color-border: ${
      isDark.value ? "hsla(0, 0%, 100%, .12)" : "hsla(0, 0%, 100%, .12)"
    };
    --color-highlight-bg: ${
      isDark.value ? "hsla(0, 0%, 100%, .12)" : "hsla(0, 0%, 100%, .12)"
    };
    --color-secondary-text: ${
      isDark.value ? "rgba(156, 163, 175, 1)" : "rgba(75, 85, 99, 1)"
    };
